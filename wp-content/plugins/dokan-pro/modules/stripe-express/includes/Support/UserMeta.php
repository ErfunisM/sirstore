...
class UserMeta {
    ...
    public static function delete_stripe_account_id( $user_id, $force = false ) {
        ...
        if ( ! $force ) {
			...
			$account_id = get_user_meta( $user_id, "{$meta_key}_trash", true );
			$deleted_from_stripe_server = apply_filters( 'dokan_stripe_express_is_vendor_stripe_account_deleted_from_remote', false, $account_id );
			if ( $deleted_from_stripe_server ) {
				delete_user_meta( $user_id, "{$meta_key}_trash" );
			}
