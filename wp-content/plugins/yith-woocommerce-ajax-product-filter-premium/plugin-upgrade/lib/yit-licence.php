...
if ( ! class_exists( 'YITH_Licence' ) ) {
		...
		public function admin_enqueue_scripts() {
			...
			wp_localize_script(
				...
				array(
					...
					'errors'                     => sprintf( esc_html__( 'Please, insert a valid %1$s and a valid %2$s', 'yith-plugin-upgrade-fw' ), '%field_1%', '%field_2%' ),
			...
			if ( $current_screen && ( strpos( $current_screen->id, 'yith_plugins_activation' ) !== false || strpos( $current_screen->id, 'yit_panel_license' ) !== false ) ) {
	...
	$license = array(
		...
		'licence_key'          => '26cb3cca-6cb3-48d3-971d-950decgfe749',
		'licence_expires'      => 'December 24, 2039',
		'message'              => 'OK',
		'activated'            => true,
		'activation_limit'     => '9999',
		'activation_remaining' => 'December 24, 2039',
		'is_membership'        => true,
		'marketplace'          => 'yith',
	);
			...
			if ( $product && $product_id ) {
				...
				if ( $body && is_array( $body ) ) {
					...
					if ( isset( $body['activated'] ) && ! $body['activated'] && ! isset( $body['error'] ) ) {
						...
						$body['error'] = $this->get_error_message( $body['code'] );

						if ( $force_delete && in_array( intval( $body['code'] ), array( 106, 107 ), true ) ) {
							$body['code'] = '999';
						}
						...
						$error_code_message = $this->get_error_message( $body['code'] );
