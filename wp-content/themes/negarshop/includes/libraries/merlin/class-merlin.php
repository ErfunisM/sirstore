...
class Merlin
{
    ...
    function required_classes()
    {
        ...
        $this->importer = new ProteusThemes\WPContentImporter2\Importer(array(
    ...
    public function redirect()
    {
        ...
        wp_redirect(menu_page_url($this->merlin_url));
        exit;
    }
    ...
    public function admin_page()
    {
        ...
        $texts = array(
            'something_went_wrong' => esc_html__('Something went wrong. Please refresh the page and try again!', 'negarshop'),
        );
        ...
        if (get_option('negarshop_installer_ready', false) !== '1') {
            echo '<div id="message" class="notice"><p>برای کار با نصب آسان ابتدا باید <b>افزونه های ضروری را نصب و فعال</b> نمایید!</p><a href="' . admin_url('themes.php?page=negarshop-install-plugins') . '" class="button">نصب افزونه های ضروری</a></div>';
            $this->footer();
            exit;
        }
        ?>
    ...
    public function svg($args = array())
    {
        ...
        if (empty($args)) {
            return __('Please define default parameters in the form of an array.', 'negarshop');
        }

        // Define an icon.
        if (false === array_key_exists('icon', $args)) {
            return __('Please define an SVG icon filename.', 'negarshop');
        }
    ...
    public function steps()
    {
        ...
        $this->steps = array(
            'welcome' => array(
                'name' => esc_html__('Welcome', 'negarshop'),
        ...
        if (class_exists('TGM_Plugin_Activation')) {
            $this->steps['plugins'] = array(
                'name' => esc_html__('Plugins', 'negarshop'),
        ...
        if (!empty($this->import_files)) {
            $this->steps['content'] = array(
                'name' => esc_html__('Content', 'negarshop'),
                'view' => array($this, 'content'),
            );
        }

        $this->steps['ready'] = array(
            'name' => esc_html__('Ready', 'negarshop'),
    ...
    protected function welcome()
    {
        ...
        <?php
        $this->logger->debug(__('The welcome step has been displayed', 'negarshop'));
    }
    ...
    protected function license()
    {
        ...
        <?php
        $this->logger->debug(__('The license activation step has been displayed', 'negarshop'));
    }
    ...
    protected function child()
    {
        ...
        <?php
        $this->logger->debug(__('The child theme installation step has been displayed', 'negarshop'));
    }
    ...
    protected function plugins()
    {
                        ...
                        <?php foreach ($required_plugins as $slug => $plugin) : ?>
                            <li data-slug="<?php echo esc_attr($slug); ?>">
                                ...
                                <label for="default_plugins_<?php echo esc_attr($slug); ?>">
                                    ...
                                    <span class="badge">
									<span class="hint--top" aria-label="<?php esc_attr_e('Required', 'negarshop'); ?>">
										<?php esc_html_e('req', 'negarshop'); ?>
									</span>
								</span>
                                </label>
                            </li>
        ...
        <?php
        $this->logger->debug(__('The plugin installation step has been displayed', 'negarshop'));
    }
    ...
    protected function content()
    {
            ...
            <?php if (1 < count($this->import_files)) : ?>
                ...
                <div class="merlin__select-control-wrapper">
                    ...
                    <div class="merlin__select-control-help">
						<span class="hint--top" aria-label="<?php echo esc_attr__('Select Demo', 'negarshop'); ?>">
        ...
        <?php
        $this->logger->debug(__('The content import step has been displayed', 'negarshop'));
    }
    ...
    protected function ready()
    {
        ...
        <?php
        $this->logger->debug(__('The final step has been displayed', 'negarshop'));
    }
    ...
    public function generate_child()
    {
        ...
        $name = $this->theme->get_template() . ' Child';
        ...
        if (!file_exists($path)) {
            ...
            $this->logger->debug(__('The existing child theme was activated', 'negarshop'));
        ...
        $this->logger->debug(__('The newly generated child theme was activated', 'negarshop'));
    ...
    public function _ajax_activate_license()
    {
        ...
        if (!check_ajax_referer('merlin_nonce', 'wpnonce')) {
            wp_send_json(
                array(
                    'success' => false,
                    'message' => esc_html__('Yikes! The theme activation failed. Please try again or contact support.', 'negarshop'),
                )
            );
        }
        ...
        if (empty($_POST['license_key'])) {
            wp_send_json(
                array(
                    'success' => false,
                    'message' => esc_html__('Please add your license key before attempting to activate one.', 'negarshop'),
                )
            );
        }
        ...
        $this->logger->debug(__('The license activation was performed with the following results', 'negarshop'), $result);
    ...
    protected function edd_activate_license($license)
    {
        ...
        if (is_wp_error($response) || 200 !== wp_remote_retrieve_response_code($response)) {
            ...
            if (is_wp_error($response)) {
                ...
                $message = esc_html__('An error occurred, please try again.', 'negarshop');
            }
        } else {
                ...
                switch ($license_data->error) {
                    ...
                    case 'expired':
                        ...
                        /* translators: Expiration date */
                            esc_html__('Your license key expired on %s.', 'negarshop'),
                            date_i18n(get_option('date_format'), strtotime($license_data->expires, current_time('timestamp')))
                        );
                        break;

                    case 'revoked':
                        $message = esc_html__('Your license key has been disabled.', 'negarshop');
                        break;

                    case 'missing':
                        $message = esc_html__('This appears to be an invalid license key. Please try again or contact support.', 'negarshop');
                        break;

                    case 'invalid':
                    case 'site_inactive':
                        $message = esc_html__('Your license is not active for this URL.', 'negarshop');
                        break;

                    case 'item_name_mismatch':
                        /* translators: EDD Item Name */
                        $message = sprintf(esc_html__('This appears to be an invalid license key for %s.', 'negarshop'), $this->edd_item_name);
                        break;

                    case 'no_activations_left':
                        $message = esc_html__('Your license key has reached its activation limit.', 'negarshop');
                        break;
    ...
    public function generate_child_functions_php($slug)
    {
        ...
        $this->logger->debug(__('The child theme functions.php content was generated', 'negarshop'));
    ...
    public function generate_child_style_css($slug, $parent, $author, $version)
    {
        ...
        $this->logger->debug(__('The child theme style.css content was generated', 'negarshop'));
    ...
    public function generate_child_screenshot($path)
    {
        ...
        if (!empty($screenshot) && file_exists($screenshot)) {
            ...
            $this->logger->debug(__('The child theme screenshot was copied to the child theme, with the following result', 'negarshop'), array('copied' => $copied));
        } else {
            $this->logger->debug(__('The child theme screenshot was not generated, because of these results', 'negarshop'), array('screenshot' => $screenshot));
        }
    }
    ...
    function _ajax_plugins()
    {
        ...
        foreach ($plugins['activate'] as $slug => $plugin) {
            if ($_POST['slug'] === $slug) {
                $json = array(
                    ...
                    'action2' => -1,
                    'message' => esc_html__('Activating', 'negarshop'),
                );
                break;
            }
        }
        ...
        foreach ($plugins['update'] as $slug => $plugin) {
            if ($_POST['slug'] === $slug) {
                $json = array(
                    ...
                    'action2' => -1,
                    'message' => esc_html__('Updating', 'negarshop'),
                );
                break;
            }
        }
        ...
        foreach ($plugins['install'] as $slug => $plugin) {
            if ($_POST['slug'] === $slug) {
                $json = array(
                    ...
                    'action2' => -1,
                    'message' => esc_html__('Installing', 'negarshop'),
                );
                break;
            }
        }
        ...
        if ($json) {
            $this->logger->debug(
                __('A plugin with the following data will be processed', 'negarshop'),
                array(
            ...
            $json['message'] = esc_html__('Installing', 'negarshop');
            wp_send_json($json);
        } else {
            $this->logger->debug(
                __('A plugin with the following data was processed', 'negarshop'),
                array(
                    'plugin_slug' => $_POST['slug'],
                )
            );
            ...
            wp_send_json(
                array(
                    'done' => 1,
                    'message' => esc_html__('Success', 'negarshop'),
                )
            );
        }
    ...
    function _ajax_content()
    {
        ...
        if (!check_ajax_referer('merlin_nonce', 'wpnonce') || empty($_POST['content']) && isset($content[$_POST['content']])) {
            $this->logger->error(__('The content importer AJAX call failed to start, because of incorrect data', 'negarshop'));

            wp_send_json_error(
                array(
                    'error' => 1,
                    'message' => esc_html__('Invalid content!', 'negarshop'),
                )
            );
        }
        ...
        if (isset($_POST['proceed'])) {
            if (is_callable($this_content['install_callback'])) {
                $this->logger->info(
                    __('The content import AJAX call will be executed with this import data', 'negarshop'),
                    array(
        ...
        if ($json) {
            ...
            $this->logger->error(
                __('The content import AJAX call failed with this passed data', 'negarshop'),
                array(
            ...
            wp_send_json(
                array(
                    'error' => 1,
                    'message' => esc_html__('Error', 'negarshop'),
                    'logs' => '',
                    'errors' => '',
                )
            );
    ...
    public function _ajax_get_total_content_import_items()
    {
        if (!check_ajax_referer('merlin_nonce', 'wpnonce') && empty($_POST['selected_index'])) {
            $this->logger->error(__('The content importer AJAX call for retrieving total content import items failed to start, because of incorrect data.', 'negarshop'));

            wp_send_json_error(
                array(
                    'error' => 1,
                    'message' => esc_html__('Invalid data!', 'negarshop'),
                )
            );
        }
    ...
    public function get_import_data_info($selected_import_index = 0)
    {
        ...
        if (
            ...
            $import_data['homepage'] = true;
        }

        if (
            !empty($this->import_files[$selected_import_index]['unyson'])
        ) {
            $import_data['unyson'] = true;
        }

        if (
            !empty($this->import_files[$selected_import_index]['header'])
        ) {
            $import_data['header'] = true;
        }

        if (
            !empty($this->import_files[$selected_import_index]['footer'])
        ) {
            $import_data['footer'] = true;
        }

        if (
            !empty($this->import_files[$selected_import_index]['pages'])
        ) {
            $import_data['pages'] = true;
        }

        if (
            !empty($this->import_files[$selected_import_index]['products'])
        ) {
            $import_data['products'] = true;
        }

        if (
            !empty($this->import_files[$selected_import_index]['media'])
        ) {
            $import_data['media'] = true;
        }

        if (
            !empty($this->import_files[$selected_import_index]['elementor_kit'])
        ) {
            $import_data['elementor_kit'] = true;
        }
    ...
    protected function get_import_data($selected_import_index = 0)
    {
        ...
        if (!empty($import_files['content'])) {
            $content['content'] = array(
                'title' => esc_html__('Content', 'negarshop'),
                'description' => esc_html__('Demo content data.', 'negarshop'),
                'pending' => esc_html__('Pending', 'negarshop'),
                'installing' => esc_html__('Installing', 'negarshop'),
                'success' => esc_html__('Success', 'negarshop'),
        ...
        if (!empty($import_files['homepage'])) {
            $content['homepage'] = array(
                'title' => esc_html__('Home Page', 'negarshop'),
                'description' => esc_html__('Demo Home page data.', 'negarshop'),
                'pending' => esc_html__('Pending', 'negarshop'),
                'installing' => esc_html__('Installing', 'negarshop'),
                'success' => esc_html__('Success', 'negarshop'),
                'checked' => $this->is_possible_upgrade() ? 0 : 1,
                'install_callback' => array($this->importer, 'import'),
                'data' => $import_files['homepage'],
            );
        }

        if (!empty($import_files['header'])) {
            $content['header'] = array(
                'title' => esc_html__('Header', 'negarshop'),
                'description' => esc_html__('Demo Header data.', 'negarshop'),
                'pending' => esc_html__('Pending', 'negarshop'),
                'installing' => esc_html__('Installing', 'negarshop'),
                'success' => esc_html__('Success', 'negarshop'),
                'checked' => $this->is_possible_upgrade() ? 0 : 1,
                'install_callback' => array($this->importer, 'import'),
                'data' => $import_files['header'],
            );
        }

        if (!empty($import_files['footer'])) {
            $content['footer'] = array(
                'title' => esc_html__('Footer', 'negarshop'),
                'description' => esc_html__('Demo Footer data.', 'negarshop'),
                'pending' => esc_html__('Pending', 'negarshop'),
                'installing' => esc_html__('Installing', 'negarshop'),
                'success' => esc_html__('Success', 'negarshop'),
                'checked' => $this->is_possible_upgrade() ? 0 : 1,
                'install_callback' => array($this->importer, 'import'),
                'data' => $import_files['footer'],
            );
        }

        if (!empty($import_files['pages'])) {
            $content['pages'] = array(
                'title' => esc_html__('Pages', 'negarshop'),
                'description' => esc_html__('Demo Pages data.', 'negarshop'),
                'pending' => esc_html__('Pending', 'negarshop'),
                'installing' => esc_html__('Installing', 'negarshop'),
                'success' => esc_html__('Success', 'negarshop'),
                'checked' => $this->is_possible_upgrade() ? 0 : 1,
        ...
        if (!empty($import_files['products'])) {
            $content['products'] = array(
                'title' => esc_html__('Products', 'negarshop'),
                'description' => esc_html__('Demo Products data.', 'negarshop'),
                'pending' => esc_html__('Pending', 'negarshop'),
                'installing' => esc_html__('Installing', 'negarshop'),
                'success' => esc_html__('Success', 'negarshop'),
                'checked' => $this->is_possible_upgrade() ? 0 : 1,
        ...
        if (!empty($import_files['media'])) {
            $content['media'] = array(
                'title' => esc_html__('Media', 'negarshop'),
                'description' => esc_html__('Demo Media.', 'negarshop'),
                'pending' => esc_html__('Pending', 'negarshop'),
                'installing' => esc_html__('Installing', 'negarshop'),
                'success' => esc_html__('Success', 'negarshop'),
                'checked' => $this->is_possible_upgrade() ? 0 : 1,
        ...
        if (!empty($import_files['elementor_kit'])) {
            $content['elementor_kit'] = array(
                'title' => esc_html__('Elementor Uikit', 'negarshop'),
                'description' => esc_html__('Elementor Uikit.', 'negarshop'),
                'pending' => esc_html__('Pending', 'negarshop'),
                'installing' => esc_html__('Installing', 'negarshop'),
                'success' => esc_html__('Success', 'negarshop'),
                'install_callback' => array($this, 'import_elementor_kit'),
                'checked' => $this->is_possible_upgrade() ? 0 : 1,
                'data' => $import_files['elementor_kit'],
            );
        }

        if (!empty($import_files['widgets'])) {
            $content['widgets'] = array(
                'title' => esc_html__('Widgets', 'negarshop'),
                'description' => esc_html__('Sample widgets data.', 'negarshop'),
                'pending' => esc_html__('Pending', 'negarshop'),
                'installing' => esc_html__('Installing', 'negarshop'),
                'success' => esc_html__('Success', 'negarshop'),
        ...
        if (!empty($import_files['sliders'])) {
            $content['sliders'] = array(
                'title' => esc_html__('Revolution Slider', 'negarshop'),
                'description' => esc_html__('Sample Revolution sliders data.', 'negarshop'),
                'pending' => esc_html__('Pending', 'negarshop'),
                'installing' => esc_html__('Installing', 'negarshop'),
                'success' => esc_html__('Success', 'negarshop'),
        ...
        if (!empty($import_files['options'])) {
            $content['options'] = array(
                'title' => esc_html__('Options', 'negarshop'),
                'description' => esc_html__('Sample theme options data.', 'negarshop'),
                'pending' => esc_html__('Pending', 'negarshop'),
                'installing' => esc_html__('Installing', 'negarshop'),
                'success' => esc_html__('Success', 'negarshop'),
        ...
        if (!empty($import_files['redux'])) {
            $content['redux'] = array(
                'title' => esc_html__('Redux Options', 'negarshop'),
                'description' => esc_html__('Redux framework options.', 'negarshop'),
                'pending' => esc_html__('Pending', 'negarshop'),
                'installing' => esc_html__('Installing', 'negarshop'),
                'success' => esc_html__('Success', 'negarshop'),
        ...
        if (!empty($import_files['unyson'])) {
            $content['unyson'] = array(
                'title' => esc_html__('Unyson Options', 'negarshop'),
                'description' => esc_html__('Unyson framework options.', 'negarshop'),
                'pending' => esc_html__('Pending', 'negarshop'),
                'installing' => esc_html__('Installing', 'negarshop'),
                'success' => esc_html__('Success', 'negarshop'),
                'install_callback' => array($this, 'import_unyson_options'),
                'checked' => $this->is_possible_upgrade() ? 0 : 1,
                'data' => $import_files['unyson'],
            );
        }

        if (false !== has_action('merlin_after_all_import')) {
            $content['after_import'] = array(
                'title' => esc_html__('After import setup', 'negarshop'),
                'description' => esc_html__('After import setup.', 'negarshop'),
                'pending' => esc_html__('Pending', 'negarshop'),
                'installing' => esc_html__('Installing', 'negarshop'),
                'success' => esc_html__('Success', 'negarshop'),
    ...
    public function import_unyson_options($file)
    {
        ...
        update_option('fw_theme_settings_options:default', maybe_unserialize($optionsData));
     ...
     * @throws Exception
     */
    public function import_elementor_kit($file)
    {
        if (!class_exists('\Elementor\App\Modules\ImportExport\Processes\Import')) {
            return 'failed';
        }
        $elementorImport = new \Elementor\App\Modules\ImportExport\Processes\Import($file);
        $elementorImport->register_default_runners();
        error_log(json_encode($elementorImport->run()));
        $this->logger->info(__('The elementor import was executed', 'negarshop'));
        if (defined('DOING_AJAX') && DOING_AJAX) {
            return 'true';
        }
    }
     ...
     * @throws Exception
     */
    public function import_elementor_templates($file)
    {
        if (!class_exists('\Elementor\App\Modules\ImportExport\Processes\Import')) {
            return 'failed';
        }

        $elementorManager = new Elementor\TemplateLibrary\Manager();
        $source = $elementorManager->get_source('local');
        $import = $source->import_template(basename($file), $file);
    ...
    public function import_revolution_sliders($file)
    {
        ...
        $this->logger->info(__('The revolution slider import was executed', 'negarshop'));
    ...
    public function pt_importer_new_ajax_request_response_data($data)
    {
        ...
        $data['message'] = esc_html__('Installing', 'negarshop');
        $data['proceed'] = 'true';
        $data['action'] = 'merlin_content';
        $data['content'] = $_POST['content'] ?? 'content';
    ...
    public function after_content_import_setup()
    {
        ...
        $customQueryArgs = [
            'post_type' => 'page',
            'title' => apply_filters('merlin_content_home_page_title', 'صفحه نخست'),
            'order' => 'DESC',
            'orderby' => 'Date'
        ];
        ...
        if ($customQuery->have_posts()) {
            ...
            $this->logger->debug(__('The home page was set', 'negarshop'), array('homepage_id' => get_the_ID()));
        }
        wp_reset_query();

        $elementor_pro_theme_builder_conditions = get_option('elementor_pro_theme_builder_conditions', []);
        // Set elementor header.
        $mainHeader = false;
        $customQueryArgs = [
            'post_type' => 'elementor_library',
            'title' => apply_filters('merlin_content_header_title', 'سربرگ'),
            'order' => 'DESC',
            'orderby' => 'Date'
        ];
        $customQuery = new WP_Query($customQueryArgs);
        if ($customQuery->have_posts()) {
            $customQuery->the_post();
            $elementor_pro_theme_builder_conditions['header'] = [];
            $mainHeader = get_the_ID();
            $elementor_pro_theme_builder_conditions['header'][get_the_ID()] = [
                "include/general"
            ];
            update_option('elementor_pro_theme_builder_conditions', $elementor_pro_theme_builder_conditions);
            $this->logger->debug(__('The elementor header was set', 'negarshop'), array('header_id' => get_the_ID()));
        }
        wp_reset_query();
        // Set elementor secondary header.
        $customQueryArgs = [
            'post_type' => 'elementor_library',
            'title' => apply_filters('merlin_content_header_title', 'سربرگ جانبی'),
            'order' => 'DESC',
            'orderby' => 'Date'
        ];
        $customQuery = new WP_Query($customQueryArgs);
        if ($customQuery->have_posts()) {
            ...
            if (false !== $mainHeader) {
                $elementor_pro_theme_builder_conditions['header'][$mainHeader] = [
                    'include/singular/front_page'
                ];
            }
            update_option('elementor_pro_theme_builder_conditions', $elementor_pro_theme_builder_conditions);
            $this->logger->debug(__('The elementor secondary header was set', 'negarshop'), array('header_id' => get_the_ID()));
        }
        wp_reset_query();

        // Set elementor mobile header.
        $customQueryArgs = [
            'post_type' => 'elementor_library',
            'title' => apply_filters('merlin_content_header_title', 'سربرگ موبایل'),
            'order' => 'DESC',
            'orderby' => 'Date'
        ];
        $customQuery = new WP_Query($customQueryArgs);
        if ($customQuery->have_posts()) {
            $customQuery->the_post();
            $elementor_pro_theme_builder_conditions['responsive-header'] = [];
            $elementor_pro_theme_builder_conditions['responsive-header'][get_the_ID()] = [
                "include/general"
            ];
            update_option('elementor_pro_theme_builder_conditions', $elementor_pro_theme_builder_conditions);
            $this->logger->debug(__('The elementor mobile header was set', 'negarshop'), array('mobile_header_id' => get_the_ID()));
        }
        wp_reset_query();

        // Set elementor footer.
        $customQueryArgs = [
            'post_type' => 'elementor_library',
            'title' => apply_filters('merlin_content_footer_title', 'پابرگ'),
            'order' => 'DESC',
            'orderby' => 'Date'
        ];
        $customQuery = new WP_Query($customQueryArgs);
        if ($customQuery->have_posts()) {
            $customQuery->the_post();
            $elementor_pro_theme_builder_conditions['footer'] = [];
            $elementor_pro_theme_builder_conditions['footer'][get_the_ID()] = [
                "include/general"
            ];
            update_option('elementor_pro_theme_builder_conditions', $elementor_pro_theme_builder_conditions);
            $this->logger->debug(__('The elementor footer was set', 'negarshop'), array('footer_id' => get_the_ID()));
        }
        wp_reset_query();
    ...
    public function before_content_import_setup()
    {
        ...
        if (!empty($hello_world)) {
            ...
            $this->logger->debug(__('The Hello world post status was set to draft', 'negarshop'));
        }
    }
    ...
    public function validate_import_file_info($import_files)
    {
        ...
        foreach ($import_files as $import_file) {
            if (!empty($import_file['import_file_name'])) {
                ...
                $this->logger->warning(__('This predefined demo import does not have the name parameter: import_file_name', 'negarshop'), $import_file);
            }
        }
    ...
    public function get_import_files_paths($selected_import_index)
    {
        ...
        if (!empty($selected_import_data['homepage']) && file_exists($selected_import_data['homepage'])) {
            $import_files['homepage'] = $selected_import_data['homepage'];
        }
        if (!empty($selected_import_data['unyson']) && file_exists($selected_import_data['unyson'])) {
            $import_files['unyson'] = $selected_import_data['unyson'];
        }
        if (!empty($selected_import_data['header']) && file_exists($selected_import_data['header'])) {
            $import_files['header'] = $selected_import_data['header'];
        }
        if (!empty($selected_import_data['footer']) && file_exists($selected_import_data['footer'])) {
            $import_files['footer'] = $selected_import_data['footer'];
        }
        if (!empty($selected_import_data['pages']) && file_exists($selected_import_data['pages'])) {
            $import_files['pages'] = $selected_import_data['pages'];
        }
        if (!empty($selected_import_data['products']) && file_exists($selected_import_data['products'])) {
            $import_files['products'] = $selected_import_data['products'];
        }
        if (!empty($selected_import_data['media']) && file_exists($selected_import_data['media'])) {
            $import_files['media'] = $selected_import_data['media'];
        }
        if (!empty($selected_import_data['elementor_kit']) && file_exists($selected_import_data['elementor_kit'])) {
            $import_files['elementor_kit'] = $selected_import_data['elementor_kit'];
        }
    ...
    public function get_import_steps_html($import_info)
    {
        ...
        <li class="merlin__drawer--import-content__list-item status status--Pending"
            ...
            <label for="default_content_<?php echo esc_attr($slug); ?>">
                <i></i><span><?php echo esc_html($this->strings['types'][$slug] ?? ucfirst(str_replace('_', ' ', $slug))); ?></span>
            </label>
        </li>
